...
-- Helper index for the function's duplicate-check lookup
CREATE INDEX IF NOT EXISTS idx_tm_verdict_judge_pair_date
  ON public.tm_verdict (judge_user_id, red_fighter_id, blue_fighter_id, signed_at);
...
BEGIN
  ...
  -- Idempotency: same judge + same pair + same UTC day already saved = duplicate
  ...
  INSERT INTO public.tm_verdict (
    ...
    v_records_updated, COALESCE(p_rounds, '[]'::jsonb)
  )
...
REVOKE EXECUTE ON FUNCTION public.save_fight_result(uuid, uuid, uuid, text, int, int, int, jsonb, text, boolean) FROM PUBLIC;
GRANT  EXECUTE ON FUNCTION public.save_fight_result(uuid, uuid, uuid, text, int, int, int, jsonb, text, boolean) TO authenticated;
