-- Ensure the RPC uses the correct, safe implementation and remove old overload
DROP FUNCTION IF EXISTS public.request_fighter_license(jsonb, jsonb);
...
BEGIN
  ...
  IF v_first_name IS NULL OR TRIM(v_first_name) = '' OR v_last_name IS NULL OR TRIM(v_last_name) = '' THEN
    RETURN jsonb_build_object('success', false, 'message', 'Por favor completa tu nombre y apellido en tu perfil de usuario antes de solicitar una licencia');
  END IF;
  ...
  INSERT INTO public.fighter_profiles (
    ...
    CASE 
      ...
      ELSE NULL
    END,
    CASE WHEN jsonb_typeof(p_fighter_profile_data->'martial_arts') = 'array' 
      THEN ARRAY(SELECT jsonb_array_elements_text(p_fighter_profile_data->'martial_arts')) ELSE NULL END,
  ...
  INSERT INTO public.fighter_licenses (
    fighter_id, license_level, status, discipline, created_by
  ) VALUES (
    v_fighter_id,
